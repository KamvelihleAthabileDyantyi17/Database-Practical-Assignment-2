...
SELECT 
    ...
    k.kayak_type, 
    u.upgrade_work, 
    c.cust_contact -- Changed this to match your table
FROM 
    kayak_upgrades ku
JOIN 
    customers c ON ku.cust_id = c.cust_id
...
JOIN 
    upgrades u ON ku.upgrade_id = u.upgrade_id
WHERE 
    UPPER(c.cust_address) LIKE '%SUMMER%';

--
SELECT * FROM vwCustUpgrades;

--
UPDATE customers SET cust_address = '42 Summer Way, Cape Town' WHERE cust_id = 1;
COMMIT;
SELECT * FROM vwCustUpgrades;